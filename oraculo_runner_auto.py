...
TENNIS_MIN_CONF = 0.76       # 2026-06-12: sub-bucket analysis — 74-76% WR=63% vs 76-78% WR=73%; cut bad range
...
SPORT_KELLY = {             # Per-sport Kelly fractions
    'tennis':     0.48,  # 2026-07-17: subido de 0.20 -- Terra tiene bankroll ~2.4x mas chico que Oraculo, con el mismo Kelly quedaba muy por debajo del techo de 4 dolares en sets_under/team_win_set. Escalado por la proporcion de bankroll para igualar el stake nominal.
...
MIN_STAKE = 2.00            # 2026-07-27: bajado a 2 -- $5 vetaba casi todo el Kelly optimo con bankroll chico ($82 vs $201 Oraculo, ratio ~2.4x)
MAX_STAKE_ABS = 10.0        # 2026-08-02: ajustado a 10 a pedido (pasó por 4 antes de esta version)
...
MLB_FADE_TEAMS = {
    ...
    'CHW White Sox',                             # 38% WR n=13 pnl=-$104 (re-added Jun-15; removed Jun-05 prematuramente)
    'MIA Marlins',                               # MIA 37% WR n=13
}
...
RECONCILE_AUTO_CORRECT_ENABLED = True  # 2026-07-06: ported from oraculo_v2 -- reconcile_bankroll()
                                        # now compares against the real Cloudbet wallet balance instead
                                        # of get_bets(limit=50)'s truncated settled-PnL sum. Terra's
                                        # bankroll was also inflated ($171.42 local vs $86.69 real) --
                                        # root cause here was a stale cumulative_void_returns=$84.73
                                        # left over from cloning oraculo_v2's state.json on 2026-06-26
                                        # (Terra has zero VOID bets in its own real history). Manually
                                        # corrected bankroll and zeroed cumulative_void_returns once;
                                        # this field is no longer used by the rewritten formula anyway.
                                        # See memory oraculo_reconcile_bankroll_bug.
...
LEAGUE_MARKETS = {
    ...
    'SB':  ['over25'],
    'FL2': ['over25'],
    'PER': ['under25', 'peru_dc'],  # 2026-06-28: U2.5=52.6% (650 matches); peru_dc added 2026-07-12
    'ESP2': ['over25'],
    'BRA2': ['over25'],
    'COL':  ['over25'],
    'CHL':  ['over25'],
...
CB_COMPS = {
    ...
    'CSL': 'soccer-china-chinese-super-league',  # 2026-06-28: post-WC
}
...
def _auto_tune_strategy(state):
    ...
    try:
        ...
        if len(baseball) >= 20:
            if bb_wr > 0.60:
                MIN_CONF = max(0.67, MIN_CONF - 0.01)  # piso 0.67: alineado con backtest threshold 70%
            elif bb_wr < 0.40:
                MIN_CONF = min(0.75, MIN_CONF + 0.01)  # cap 0.75: backtest-calibrated upper bound
        elif len(recent) >= 30 and len(baseball) < 20:
            # Fall back to overall WR only if insufficient baseball sample
            if win_rate > 0.75:
                MIN_CONF = max(0.67, MIN_CONF - 0.01)
            elif win_rate < 0.40:
                MIN_CONF = min(0.75, MIN_CONF + 0.01)
...
def scan_football(api, state, dry_run=False):
    ...
    try:
        ...
        # 2026-08-01 fix (replicado de oraculo_v2): build_feature_vector hace
        # elo.ratings[team] (acceso directo, no .get()) -- KeyError silencioso
        # para equipos no vistos por el retrain (nombres crudos del CSV).
    ...
    # 2026-08-01 (replicado): contexto historico para build_feature_vector
    # (105 features). Cacheado internamente (TTL 24h).
    _fb_context = None
    try:
    ...
    for league, comp_key in CB_COMPS.items():
        ...
        for ev in events:
            ...
            # 2026-08-01 perf (replicado): calcular historial UNA vez por partido.
            ...
            for mkt_key in league_mkts:
                ...
                # 2026-07-12: ported from oraculo_v2, live from day 1 (max_stake capped, no shadow phase)
                        ...
                        if _hd_price and _hd_price > 1.05 and _hd_murl:
                            ...
                            if _dc_edge >= 0.04 and _p1x >= 0.76:  # 2026-07-19: subido de 0.70 -- con la base nueva (0.74) el piso viejo nunca fallaba
                                picks.append({
                                    ...
                                    'edge': _dc_edge,
                                    'sport': 'soccer',
                                    '_max_stake': 2.00,  # 2026-07-12: live, capped at 2 USD
                ...
                if mp and _fb_context:
                    try:
                        if not _home_recent and not _away_recent:
                            pass  # sin historial CSV -- dejar que otros fallbacks lo intenten
                        else:
                ...
                if model_prob_over is None and mkt_key == 'over25' and poisson:
                    try:
                        ...
                        pass
                # 2026-08-01 (replicado): fallback de tasa historica de goles para
                # ARG/BRA en over15/under35 -- over25 NO se usa (backtest mostro
                # que no aporta señal real para estas ligas).
                ...
                for outcome, params_filter in outcomes:
                    ...
                    if edge > MIN_EDGE and prob > MIN_CONF and edge < 0.45 and prob < 0.92:
                        ...
                        if mkt_key in ('btts_yes', 'btts_no'):
                            ...
                            # 2026-08-01 (replicado): modelo ML de futbol estaba roto
                            # para TODAS las ligas -- shadow-only hasta validar WR.
    ...
    if intl_elo:
        ...
        _INTL_DC_LEAGUES = frozenset({'FIFA_WC', 'UEFA_NL', 'COPA_AM', 'CONMEBOL_WCQ', 'UEFA_EURO'})
            ...
            for ev in events:
                ...
                for sub_val in ou_mkt.get('submarkets', {}).values():
                    for s in sub_val.get('selections', []):
                        ...
                        if '3.5' in param and out == 'over' and price > 1.1:
                            odds_ou35['over'] = (price, murl)

                # Get BTTS odds
                odds_btts = {}
                if intl_league in _INTL_DC_LEAGUES:
                ...
                try:
                    ...
                    _use_dc = intl_league in _INTL_DC_LEAGUES and hasattr(intl_elo, 'predict_match_wc')
                    _is_cantera = _use_dc and not _is_wc
                    if _use_dc:
                        ...
                        # 2026-06-16: O/U usa xG ajustado por lesiones (antes _xg_h crudo, ciego a lesiones)
                        try:
                        ...
                        # xG efectivo O/U: lesiones + calib WC (1.043 esta en _xg_h pero no en _xg_h_adj)
                        _cal_ou = 1.043 if _is_wc else 1.0
                        _xg_h_eff = _xg_h_adj * _cal_ou
                        _xg_a_eff = _xg_a_adj * _cal_ou
                        _pu25 = sum(
                            _m.exp(-_xg_h_eff) * _xg_h_eff**_i / _m.factorial(_i) *
                            _m.exp(-_xg_a_eff) * _xg_a_eff**_j / _m.factorial(_j)
                            for _i in range(4) for _j in range(4) if _i + _j <= 2
                        )
                        p_o25 = max(0.0, min(1.0, 1.0 - _pu25))
                        # Over 3.5 Poisson (WC blowout shadow tracking — backtest WC2022: P>=62% WR=75%)
                        _pu35 = sum(
                            _m.exp(-_xg_h_eff) * _xg_h_eff**_i / _m.factorial(_i) *
                            _m.exp(-_xg_a_eff) * _xg_a_eff**_j / _m.factorial(_j)
                            for _i in range(6) for _j in range(6) if _i + _j <= 3
                        )
                        p_o35 = max(0.0, min(1.0, 1.0 - _pu35))
                        # BTTS from player-adjusted DC xG (injury factors applied) — usa _xg_h_adj (sin calib, conservador, post-WC unificar)
                        p_btts_yes = max(0.0, min(1.0, (1 - _m.exp(-_xg_h_adj)) * (1 - _m.exp(-_xg_a_adj))))
                        p_btts_no  = 1.0 - p_btts_yes
                        # 2H goals: ~45% of player-adjusted xG
                        _xh2, _xa2 = _xg_h_adj * 0.54, _xg_a_adj * 0.54  # 2026-06-12: Fable audit 2H=54% goals not 45%
                        _pu15_2h = sum(
                        ...
                        # Mismatch level for Under 2H guard (set below)
                        try:
                            from oraculo_wc_model import get_elo_mismatch as _gem
                            _, _wc_mm_lvl = _gem(home, away)
                        except Exception:
                            _wc_mm_lvl = 0
                    else:
                        _is_cantera = False
                        # 2026-08-02: portado de Oraculo -- IntlElo solo tiene selecciones
                        # nacionales (282 equipos, 0 clubes) -- Champions/Europa/Conference
                        # League nunca generaban pick real porque ambos equipos caian al
                        # rating por defecto (50/50, sin edge), en silencio. Usar el modelo
                        # de clubes (poisson, entrenado con ligas reales) cuando conoce a
                        # alguno de los dos equipos del partido.
                ...
                # 2026-06-15: also block Under 2H on ALL round-3 games (motivation dynamics change —
                # teams attacking for classification inflate 2H goals unpredictably)
                _skip_1x2 = False
                _skip_btts_yes = False
                _skip_under_2h = False
                if intl_league == 'FIFA_WC' and _wc_collusion:
                    ...
                    for _cgv in _wc_collusion.values():
                        if ((_ev_teams == set(_cgv.get('round3_game1', []))) or
                            ...
                            _skip_under_2h = True   # round-3: always block Under 2H
                            ...
                            if _pdis >= 0.08:
                                ...
                                log.info('WC collusion guard %s p_disgrace=%.3f — 1X2+BTTS-Yes skipped',
                                         match_label, _pdis)
                            log.info('WC round-3 Under 2H blocked: %s (motivation dynamics)', match_label)
                            break
                    ...
                    if out_key in odds_1x2:
                        ...
                        if _r1x2_ok and not _skip_1x2 and _wc_1x2_gate and edge >= _e_min and prob >= _c_min and edge < 0.45 and prob < 0.92 and not _use_club_model:
                            ...
                            if _is_wc:
                                ...
                                _pick['_ob_ref_p'] = round(_ob_ref_p, 4) if _ob_ref_p is not None else None
                            picks.append(_pick)
                            ...
                            # 2026-08-02: portado de Oraculo -- mismo guard shadow-only que el OU,
                            # el modelo de clubes no tiene WR validado todavia.
                            ...
                            _sibila_record({'match': match_label, 'league': intl_league,
                                'event_id': eid, 'market_url': murl,
                                'price': price, 'label': out_key.title() + ' Win (DC-cantera)',
                                'model_prob': prob, 'edge': edge,
                                'sport': 'soccer', 'intl': True, '_shadow_only': True})
                            ...
                            if (prob >= _wc_cfg2['min_prob'] and price >= _wc_cfg2['min_odds']
                                ...
                                _sibila_record({"match": match_label, "league": intl_league,
                                    "event_id": eid, "market_url": murl,
                                    "price": price, "label": out_key.title() + " Win (WC-cantera)",
                                    "model_prob": prob, "edge": edge,
                                    "sport": "soccer", "intl": True, "_shadow_only": True,
                                    "market_type": "result_1x2"})
                    ...
                    if ou_key in odds_ou:
                        ...
                        if edge >= MIN_EDGE and prob >= MIN_CONF and edge < 0.45 and prob < 0.92:
                            ...
                            if _is_cantera:
                                ...
                                # 2026-08-02: portado de Oraculo -- modelo de clubes recien
                                # conectado, shadow-only hasta acumular muestra y confirmar WR.
                                _ou_pick['_shadow_only'] = True
                ...
                if _is_wc and 'over' in odds_ou35 and _SIBILA_ENABLED:
                    ...
                    _e35 = p_o35 * _pr35 - 1
                    if p_o35 >= WC_MIN_CONF and _e35 >= WC_MIN_EDGE and _e35 < 0.45:
                        _sibila_record({
                            'match': match_label, 'league': intl_league,
                            'event_id': eid, 'market_url': _mu35,
                            'price': _pr35,
                            'label': f'Over 3.5 Goals (xG {_xg_h:.1f}+{_xg_a:.1f})',
                            'model_prob': p_o35, 'edge': _e35,
                            'sport': 'soccer', 'intl': True, '_shadow_only': True,
                            'market_type': 'wc_over35'
                        })
                        log.info('[WC O3.5 shadow] %s | P=%.0f%% odds=%.2f edge=%.1f%%',
                                 match_label, p_o35*100, _pr35, _e35*100)

                # BTTS edges (DC xG model)
                if intl_league in _INTL_DC_LEAGUES and p_btts_yes is not None:
                    for _bo, _bp in [('yes', p_btts_yes), ('no', p_btts_no)]:
                        if _bo in odds_btts:
                            _pr, _mu = odds_btts[_bo]
                            _edge = _bp * _pr - 1
                            _skip_this_btts = _skip_btts_yes and _bo == 'yes'
                            _e_btts = WC_MIN_EDGE if _is_wc else MIN_EDGE
                            if not _skip_this_btts and _edge >= _e_btts and _bp >= 0.55 and _edge < 0.45 and _bp < 0.92:
                                _btts_pick = {
                                    'match': match_label, 'league': intl_league,
                                ...
                                if _is_wc: _btts_pick['_wc_phase_c'] = True
                                if _is_cantera:
                                    _btts_pick['_shadow_only'] = True
                                    _sibila_record(_btts_pick)
                                else:
                                    picks.append(_btts_pick)

                # 2026-06-17: BTTS high-xG segment SHADOW (cantera). Backtest 5y: ambas xG>=1.4 -> BTTS 56%% (+EV si odds>=1.80).
                # Valida si el mercado subvalua BTTS en partidos de ataque mutuo. NO live hasta n>=40 WR>=56%%.
                if (p_btts_yes is not None and 'yes' in odds_btts and _SIBILA_ENABLED
                        and _xg_h >= 1.4 and _xg_a >= 1.4):
                    _bx_pr, _bx_mu = odds_btts['yes']
                    if _bx_pr >= 1.80:
                        _sibila_record({'match': match_label, 'league': intl_league, 'event_id': eid,
                                        'market_url': _bx_mu, 'price': _bx_pr, 'label': 'BTTS Yes [highxG seg]',
                                        'model_prob': 0.56, 'edge': round(0.56*_bx_pr - 1, 4),
                                        'sport': 'soccer', 'intl': True, '_shadow_only': True,
                                        'market_type': 'btts_highxg'})

                # 2H Goals O/U 1.5 edges (DC xG × 0.45)
                if intl_league in _INTL_DC_LEAGUES and p_o15_2h is not None:
                    for _ho, _hp in [('over', p_o15_2h), ('under', p_u15_2h)]:
                        # 2026-06-15: block Under 2H on strong ELO mismatches (blowout risk)
                        if _ho == 'under' and _is_wc and _wc_mm_lvl >= 2:
                            import logging as _log2
                            _log2.getLogger(__name__).info(
                                '[WC Under 2H] SKIPPED strong mismatch (ELO diff>=300): %s', match_label)
                            continue
                        # 2026-06-15: block Under 2H on round-3 WC games (classification dynamics)
                        if _ho == 'under' and _is_wc and _skip_under_2h:
                            log.info('[WC Under 2H] SKIPPED round-3 game: %s', match_label)
                            continue
                        if _ho in odds_2h:
                            _pr, _mu = odds_2h[_ho]
                            _edge = _hp * _pr - 1
                            _e_2h = WC_MIN_EDGE if _is_wc else MIN_EDGE
                            if _edge >= _e_2h and _hp >= 0.55 and _edge < 0.45 and _hp < 0.92:
                                _2h_pick = {
                                    'match': match_label, 'league': intl_league,
                                    'event_id': eid, 'market_url': _mu,
                                    'price': _pr,
                                    'label': f'Goals 2H {_ho.title()} 1.5 (xG {_xg_h*0.54:.1f}+{_xg_a*0.54:.1f})',
                                    'model_prob': _hp, 'edge': _edge,
                                    'sport': 'soccer', 'intl': True,
                                }
                                if _is_wc: _2h_pick['_wc_phase_c'] = True
                                if _is_cantera:
                                    _2h_pick['_shadow_only'] = True
                                    _sibila_record(_2h_pick)
                                else:
                                    picks.append(_2h_pick)
    ...
    if _FRESH_ENABLED:
        ...
        if _fs: log.info(_fs)
    # Benter trick: recalibrar model_prob con implied del bookmaker (UNA vez)
    # 2026-06-17: era 4x consecutivo (copy-paste) -> aplastaba edge (Argentina 2H Under 21.7%->2.8%). Fix: 1x.
    if _BENTER_ENABLED:
...
def scan_tennis(api, state, dry_run=False):
    ...
    COMP_SURFACE = {
        ...
        'london': 'grass', 'mallorca': 'grass', 'bad-homburg': 'grass',
        'nottingham': 'grass', 'birmingham': 'grass',  # stuttgart omitido: WTA Porsche es clay
        ...
        # 2026-06-17: Sudamerica = clay (challengers clay mal clasificados como hard, ej. Asuncion/Coria)
        'paraguay': 'clay', 'brazil': 'clay', 'argentina': 'clay', 'chile': 'clay',
        'peru': 'clay', 'colombia': 'clay', 'bolivia': 'clay', 'ecuador': 'clay',
        'uruguay': 'clay', 'guayaquil': 'clay', 'lima': 'clay', 'bogota': 'clay',
        'challenger': 'hard',  # fallback: resto de challengers asumido hard
    }

    # 2026-06-16: token-boundary match — 'halle' substring de 'challenger' clasificaba
    # TODOS los challengers como grass. Envolver en '-' evita el falso match.
    def _detect_surface(ck):
        ck = (ck or '').lower()
        _wrapped = '-' + ck + '-'
    ...
    for comp_key in tennis_comps:
        ...
        for ev in events:
            ...
            try:
                ...
                _surf = _detect_surface(comp_key)
                if _adv:
            ...
            try:
                ...
                prob_home = min(prob_home, 0.80)  # 2026-06-12: post-Platt >80% flat WR=71% overconfident
            except Exception:
                pass
                ...
                for sv in sets_mkt.get('submarkets', {}).values():
                    for sel in sv.get('selections', []):
                        ...
                        if '2.5' in params or outcome in ('under', 'over'):
                            try:
                                _surf_su = _detect_surface(comp_key)
        ...
        for comp_key in _p2_comps:
            ...
            for ev in events:
                ...
                if home_mc < 15 or away_mc < 15:
                    continue
                try:
                    _surf_p2 = _detect_surface(comp_key)
    ...
    try:
        ...
        if _wnba_active and _wnba_elo and len(_wnba_elo.ratings) >= 8:
            # 2026-07-06: ported from oraculo_v2 -- promoted to live there at cantera 19/20 WR=68%.
            # Terra's own WNBA cantera: n=8, WR=66.7% (smaller sample, same ballpark). $1 stake
    ...
    # --- MMA (UFC) SCANNING ---
    # 2026-07-27: ported from oraculo_v2 -- SHADOW ONLY, no colocar apuestas reales.
    # Elo entrenado con datos de ESPN (mismo modelo que Oraculo), sin validacion
    ...
    # --- NFL SCANNING ---
    # 2026-08-04: ported from oraculo_v2 -- SHADOW ONLY, no colocar apuestas reales.
    # Elo entrenado via ESPN (mismo modelo que Oraculo), sin validacion en cantera
    # todavia. NO promover a live sin pasar el mismo checklist que WNBA/MMA/sets_under
    # (20+ picks, WR>=umbral). Temporada NFL es sept-feb, fuera de esa ventana el scan
    # simplemente no encuentra eventos con cutoffTime en rango (no rompe nada, solo 0 picks).
    try:
...
def place_bets(api, state, picks, parlays, dry_run=False):
    ...
    try:
        ...
        _sc = _sl3.connect(os.path.join(SCRIPT_DIR, "sibila.db"))
        _sql = "SELECT DISTINCT match FROM sibila_picks WHERE placed=1 AND (result IS NULL OR result='VOID') AND ts > '2026-05-01'"
        for (_sm,) in _sc.execute(_sql):
            _sibila_open_matches.add(_sm)
        _sc.close()
    except Exception:
        pass
    ...
    for p in picks[:MAX_BETS_PER_SCAN]:
        if straight_remaining < 1.00:  # 1.00 = min tennis cap (was MIN_STAKE=2.00)
            break
        ...
        if p.get('event_id') and str(p.get('event_id')) in _restricted_evs:
            ...
            continue
        # Skip if sibila has an open live bet on this match (robust cross-cycle dedup)
        if p.get('match') in _sibila_open_matches:
            log.info('  [SKIP-SIBILA] Open bet in sibila: %s', p['match'][:35])
            continue
        ...
        stake = min(stake, MAX_STAKE_ABS)  # global hard cap
        ...
        if _wc_reserve > 0 and _bet_currency == 'USDC':
            ...
            _is_wc_pick = p.get('sport', '') in ('soccer', 'football') and (p.get('league') in ('FIFA_WC', 'soccer-international-world-cup'))  # 2026-06-16: por liga, no fecha (evita que Copa/intl no-WC bypaseen la reserva)
...
def check_results(api, state):
    ...
    # sin verificar contra Cloudbet -- el 9-jul (Oraculo, mismo codigo) una
    # apuesta que seguia pendiente de verdad genero cientos de creditos
    # repetidos porque igual pasaba el umbral de 24h cada ciclo.
    # reconcile_bankroll() lo corrigio ese mismo dia, pero la causa de fondo
    # no estaba arreglada. Ahora se chequea el estado real del evento en
    # Cloudbet antes de acreditar: solo se considera fantasma si el evento no
    # existe (404, huerfano real) o paso mas de 48h del cutoff sin que
    # Cloudbet lo resuelva (probable abandono, no liquidacion lenta normal).
    from datetime import timedelta as _td
    _phantom_cut = (datetime.utcnow() - _td(hours=24)).isoformat()
    _all_active = state.get('active_bets', [])
    _phantom_candidates = [b for b in _all_active
    ...
    if _phantom_candidates:
        ...
        for _b3 in _phantom_candidates:
            ...
            if _is_phantom:
                _phantom_bets.append(_b3)
    _ab_before = len(_all_active)
    state['active_bets'] = [b for b in _all_active if b not in _phantom_bets]
    _ab_removed = _ab_before - len(state['active_bets'])
    if _ab_removed > 0:
        _phantom_total = sum(b.get('stake', 0) for b in _phantom_bets)
        _today_str = datetime.utcnow().isoformat()[:10]
        _phantom_today = sum(b.get('stake', 0) for b in _phantom_bets
        ...
        log.warning('Phantom cleanup: %d bet(s) eliminados, restored $%.2f', _ab_removed, _phantom_total)
        save_state(state)
...
def reconcile_bankroll(api, state):
    ...
    2026-07-06: rewritten (ported from oraculo_v2 after the same bug was found
    there). The previous version recomputed 'correct_bankroll' from
    get_bets(limit=50)'s settled P&L -- that call silently truncates to the
    50 MOST RECENT bets, so all_settled_pnl was never the true all-time PnL.
    As older losing bets aged out of the 50-item window and newer winners
    entered, the 'correction' drifted the bankroll upward every cycle on a
    sliding-window artifact, not real money. Now compares against the REAL
    Cloudbet wallet balance (ground truth, needs no bet-history reconstruction)
    instead. total_pnl is no longer touched here -- it's already tracked
    correctly per-bet at settlement time, so re-deriving it from the same
    ...
    try:
        ...
        if drift > 1.50:  # raised from 0.25 to kill ±$3 tennis-bet oscillation
            if RECONCILE_AUTO_CORRECT_ENABLED:
                ...
                if drift > 5.0:
                    _rmsg = ('Terra RECONCILE ALERT | drift=$%.2f'
                             ' local=$%.2f -> correct=$%.2f'
                             ' wallet=$%.2f active=$%.2f') % (
        ...
        for ab in state.get('active_bets', []):
            ...
            # Never prune bets tracked in sibila (WC and other long-horizon bets)
...
def _capture_closing_odds(api, state):
    ...
    try:
        ...
        for bet in state.get('active_bets', []):
            # 2026-06-17 CLV fix: re-capturar cada ciclo (no lockear en la 1a captura).
            # La ultima captura antes del kickoff = linea de cierre real. Si el mercado
            # ya no existe (post-kickoff), el match falla y se conserva el ultimo valor bueno.
...
try:
    ...
    try:
        from oraculo_sibila import resolve_shadow_picks as _sibila_resolve_shadow
    except ImportError:
...
def _health_check(state):
    """Tier 1 robustez (2026-06-17): detecta fallos silenciosos, alerta Telegram 1x/dia."""
    try:
        issues = []
        if not _SIBILA_ENABLED:
            issues.append('SIBILA DESACTIVADA — no se graban/resuelven picks (ceguera tipo Jun 3-16)')
        else:
            try:
                import sqlite3 as _sq
                _c = _sq.connect(os.path.join(SCRIPT_DIR, 'sibila.db'))
                _last = _c.execute('SELECT MAX(ts) FROM sibila_picks').fetchone()[0]
                _c.close()
                if _last:
                    _ah = (datetime.now() - datetime.fromisoformat(_last)).total_seconds() / 3600.0
                    if _ah > 18:
                        issues.append('Sibila sin grabar hace %.0fh' % _ah)
            except Exception:
                pass
        try:
            _pf = os.path.join(SCRIPT_DIR, 'wc2026', 'wc_player_factors.json')
            if os.path.exists(_pf):
                _ah2 = (time.time() - os.path.getmtime(_pf)) / 3600.0
                if _ah2 > 18:
        ...
        log.debug('health_check error: %s', _he)

def run_cycle(dry_run=False):
    """Single scan + place + check cycle."""
    reconcile_engine_state()
    state = load_state()
    _health_check(state)  # Tier 1: alerta fallos silenciosos
    api = CloudbetAPI()
    ...
    if not SOCCER_ENABLED:
        football_picks = []
    # 2026-08-01 (replicado de oraculo_v2): Argentina/Brasil agregadas a
    # xg_matches mientras Europa esta en receso -- shadow-only hasta validar WR.
    ...
    if football_picks and _SIBILA_ENABLED:
        # 'not p.get("_shadow_only")' evita grabar dos veces (ya se graba adentro
        # de scan_football() via su propio flag para over15/over25/under35).
    ...
    # sets_under CANTERA (grass/hard) — 2026-06-15: 1W/10L all clay (RG); track on grass to validate before unblocking live
    _su_shadow = [p for p in tennis_picks
                  if p.get('market_type') == 'sets_under'
                  and float(p.get('edge', 0) or 0) >= 0.10
                  and float(p.get('model_prob', 0) or 0) >= 0.55
                  and p.get('surface', 'hard') != 'clay'
                  and 'challenger' not in (p.get('league', '') or '').lower()]  # 2026-06-16: tour-level only (decision Wimbledon)
    log.info('[sets_under CANTERA] %d grass/hard picks found (surface filter)', len(_su_shadow))
    if _su_shadow and _SIBILA_ENABLED:
        for _sup in _su_shadow:
            _sibila_record(_sup)
        log.info('[sets_under CANTERA] %d picks shadowed to Sibila (no live)', len(_su_shadow))
    ...
    if TENNIS_CALIBRATION_LIVE:
        ...
        for _tp in tennis_picks:
            # 2026-07-13: ported from oraculo_v2 -- was hardcoded 2.00, out of
            # sync with MIN_STAKE if that ever gets raised without updating this.
            # 2026-07-31 [fix]: a sets_under le faltaba estar en esta lista -- quedaba
            # fijo en $1.00 (el 'else' generico) sin importar el MIN_STAKE real de
            # Terra ($2), mientras Oraculo si escala sets_under con su propio
            # MIN_STAKE. Divergencia accidental entre los dos bots, no intencional.
    ...
    try:
        ...
        if _mlb_elo and len(_mlb_elo.ratings) >= 20:
            ...
            from datetime import datetime as _dt
            _today_dow = _dt.now().weekday()  # 0=Mon 3=Thu
            # 2026-06-13: Thu-only for F5 ML (WR=44% Thu vs 22% rest). F5 total unrestricted.
            mlb_picks = [p for p in _all_mlb
                         if ((p.get('market_type') == 'mlb_f5_total' and MLB_F5_TOTAL_ENABLED) or
                             (p.get('market_type') == 'mlb_f5_ml' and MLB_F5_ML_ENABLED and _today_dow != 3))
            ...
            if mlb_picks:
                ...
                if _n_before_decorr != len(mlb_picks):
                    log.info('MLB [de-corr]: %d -> %d picks (best-edge per game/market)',
                             _n_before_decorr, len(mlb_picks))
            # 2026-08-01 fix (replicado): shadow-record SIEMPRE los picks que
            # pasan el pre-filtro (f5_ml y f5_total, live o no) -- sin esta fila
            # antecedente, mark_placed() no puede marcar placed=1 real cuando el
            # mercado esta 'live'. record_pick() ya deduplica.
            if _SIBILA_ENABLED:
            ...
            if not MLB_F5_ML_ENABLED and _SIBILA_ENABLED:
                _f5_shadow = [p for p in _all_mlb
                              if p.get('market_type') == 'mlb_f5_ml'
                              and float(p.get('edge') or 0) >= MLB_F5_ML_MIN_EDGE
                              and float(p.get('model_prob') or 0) >= 0.55
                              and _today_dow != 3]  # 2026-06-15: excluir Thu (44.7% WR) Mie=70.5% Vie=66.7%
                if _f5_shadow:
                    log.info('[MLB-CANTERA] %d F5 ML picks (shadow only):', len(_f5_shadow))
                    for _sp in _f5_shadow:
                        log.info('  [MLB-CANTERA] %s | edge=%.1f%% prob=%.0f%% @%.2f',
                                 _sp.get('match','?')[:38], float(_sp.get('edge',0))*100,
                                 float(_sp.get('model_prob',0))*100, float(_sp.get('price',0)))
                        _sibila_record(_sp)
    ...
    try:
        ...
        if _darts_picks:
            ...
            if _SIBILA_ENABLED:
                for _dp in _darts_picks:
                    _sibila_record(_dp)
            # darts: shadow-only (grabado en Sibila arriba). Placement live NO implementado
            # (scan_darts no construye market_url). Antes: picks.extend(_darts_picks) -> NameError silencioso (picks no existe en run_cycle)
        else:
            log.debug("Darts: 0 value picks")
    except Exception as _de:
        log.debug("Darts scan error: %s", _de)

    # 3f. Rugby: NRL promovido a vivo 2026-08-01 -- ported de oraculo_v2 (26/30
    # picks Cantera, WR=58%% shadow, bajo el umbral formal de 30 pero decision
    # explicita del usuario de arrancar igual con stake de validacion fijo de
    # 1 USD. MLR sigue en shadow puro (backtest 66.0%% acc, sin CLV+ todavia).
    rugby_picks = []
    try:
    ...
    if mlb_picks:
                    ...
                    if _fip_m and len(_match_parts) == 2:
                        ...
                        if _picked_m:
                            ...
                            if _opp_name in MLB_FADE_TEAMS and _opp_fip <= 3.20:
                                ...
                                continue
                            # Tag: FIP advantage demasiado grande (20% WR n=5) -- track sin skip hasta n>=20
                            _our_fip = _home_fip if _picked.lower() == _home_team.lower() else _away_fip
                            _fip_advantage = _opp_fip - _our_fip  # positivo = nuestro pitcher mejor
                            if _fip_advantage > 2.0:
                                log.info('MLB [fip-advantage-large %s]: FIP diff=+%.2f>2.0 -- 20%% WR zone (n=5), tracking',
                                         _picked[:12], _fip_advantage)
        ...
        for _mp in mlb_picks:
            # 2026-06-12: disabled — do not re-inflate to uncalibrated prob at sizing
            # if _mp.get('raw_model_prob_uncal'): _mp['model_prob'] = _mp['raw_model_prob_uncal']
        ...
        # 2026-08-01 fix (replicado de oraculo_v2): el strip de abajo era
        # incondicional y anulaba el propio Filtro 3 v5 que reactivo Under4.5
        # F5-total (WR=67%% n=246 Sibila) -- exceptuar esos picks (no-shadow).
    ...
    try:
        ...
        _goals_comps = [c for c in (
            state.get('_soccer_comps') or [
                ...
                # MLS cantera 2026-06-10 -- shadow hasta N>=30 picks resueltos (pausa WC, reanuda ago 2026)
                'soccer-usa-mls',
            ]
        ...
        if _goal_picks:
            ...
            if SOCCER_GOALS_ENABLED:
                ...
                # Shadow-only comps: registrar en Sibila, no apostar hasta validar N>=30
                _INTL_SHADOW_COMPS = {
                    'soccer-international-conmebol-copa-america',
                    'soccer-international-uefa-nations-league',
                ...
                # Skip picks with kickoff beyond window (WC: 80h, others: 55h)
                from datetime import datetime as _dt, timezone as _tz, timedelta as _td
                _now_utc = _dt.now(_tz.utc)
                def _kicks_within_55h(p):
                    ct = p.get('cutoff_time', '')
                    if not ct:
                        return True
                    try:
                ...
                _gp_csv = [p for p in _gp_csv if _kicks_within_55h(p)]
                if _n_before_48h > len(_gp_csv):
                    log.info('[Soccer Goals] %d picks held (kickoff >window), %d within window', _n_before_48h - len(_gp_csv), len(_gp_csv))
                if _gp_csv:
                    ...
                    # 2026-07-06: ported from oraculo_v2 -- SPORT_KELLY['soccer_under']=0.25 was
                    # backtested on FULL-TIME Under but the tag matched ANY "under" label; in
                    # practice on the main Oraculo account every real bet tagged so far was 2H
                    # Under (12/12, WR=50%, -40.9% ROI), which never had that backtest support.
                    # 2H/second-half Under now keeps the base 'soccer' Kelly (0.20).
                    for _gp2 in _gp_csv:
                    ...
                    for _gp2 in _gp_csv:
                        _gp2.setdefault('_force_validation', True)  # Jun 28: cal factor may deflate prob->Kelly=0
                    _n_placed_goals = 0
                    try:
    ...
    # 2026-06-18: CLV tambien para picks shadow/cantera (medibles antes de live)
    try:
        from oraculo_clv import CLVOracle as _CLVShadow
        _CLVShadow(odds_api_key='').record_shadow_clv(api, os.path.join(SCRIPT_DIR, 'sibila.db'))
    ...
    log.info('Cycle complete: %d picks found (%d fb + %d tn + %d mlb + %d sc + %d rug), %d placed | Bankroll: $%.2f',
...
def _startup_self_check():
    ...
    2026-07-06: ported from oraculo_v2, added there after finding two safety
    features (clv_report, the rugby resolver) silently dead for weeks -- an
    ImportError inside a try/except caught by 'except Exception: log.debug(...)'
    every cycle, invisible unless someone was reading debug-level logs. This
    runs ONCE at startup and alerts loudly (log.error + Telegram) instead of
    failing quiet. Uses importlib on modules already imported earlier in this
    file, so this is a cache lookup only -- no re-execution of module-level
    code, no new network calls, no side effects.
    """
    import importlib
    checks = [
    ...
    if problems:
        msg = 'TERRA STARTUP CHECK FAILED:\n' + '\n'.join(problems)
        log.error(msg)
        try:
            send_telegram(msg)
        except Exception:
    ...
    while True:
        ...
        try:
            ...
            if now - last_scan >= SCAN_INTERVAL:
                run_cycle()
                gc.collect()  # free memory after heavy scan
                last_scan = now
...
if __name__ == '__main__':
    # Guard: kill orphan/nohup runners with same script (prevents double-betting)
    ...
    try:
        ...
        for _pid in _procs:
            if int(_pid) != _mypid:
                ...
                log.warning('Killed orphan runner PID %s', _pid)
    except Exception:
        pass
