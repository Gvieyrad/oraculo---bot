...
MIN_STAKE = 0.50            # 2026-08-05: Rock -- ajustado a 0.50 a pedido.
MAX_STAKE_ABS = 1.50        # 2026-08-05: Rock -- subido de 1.00 a 1.50 a pedido.
...
CB_COMPS = {
    ...
    'MLS': 'soccer-usa-major-league-soccer',  # 2026-08-05 fix: key equivocado (soccer-usa-mls no existe en Cloudbet) -- mismo fix aplicado en Oraculo/Terra/Chris.
...
def scan_tennis(api, state, dry_run=False):
    ...
    # --- EUROLEAGUE SCANNING ---
    # 2026-08-04: nuevo mercado, Fase 3 del plan de expansion de deportes,
    # SHADOW ONLY -- no colocar apuestas reales. Elo entrenado via API oficial
    # de EuroLeague (api-live.euroleague.net), sin validacion en cantera
    # todavia. NO promover a live sin pasar el mismo checklist que
    # WNBA/MMA/NFL/sets_under (20+ picks, WR>=umbral). Cloudbet solo tiene
    # mercado de outrights ahora (temporada oct-mayo, pausa en agosto) -- el
    # scan simplemente no encuentra mercado de partido todavia, no rompe nada.
    try:
...
def run_cycle(dry_run=False):
    ...
    if TENNIS_CALIBRATION_LIVE:
        ...
        tennis_picks = [p for p in tennis_picks
                        ...
                        # sets_under: REVERTIDO a shadow-only 2026-08-05 -- ROI real -10.0%% sobre
                        # n=241 resueltos (jun -21.5%%, jul -4.9%%, ago -15.2%%), el n=31/WR=65%% que
                        # promovio a vivo el 2026-07-05 fue racha favorable en mercado de EV negativo
                        # de fondo. Grand Slam grass: WR=41.2%% ROI=-40.1%% (n=34). Mismo revert ya
                        # aplicado en Oraculo/Terra, Rock estaba en rama separada y no lo tenia.
