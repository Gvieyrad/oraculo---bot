...
def _fetch_closing_odds(match: str, sport: str, entry_ts: str, label: str = '') -> float:
    ...
    try:
        ...
        # 2026-07-31 fix: pick the side we actually bet (via label text) instead
        # of always defaulting to home_q -- was silently fetching the WRONG
        # player's closing price for every pick on the second-listed ('away')
        # side, producing nonsense CLV (found auditing tennis_team_win_set /
        # wnba_1x2 CLV alerts: closing odds 2-4x entry odds on away-side picks).
