...
def mark_placed(match: str, label: str, bet_id: str, real_stake: float,
    ...
    already been resolved by the time this ran -- causing ~57%% of real
    bets to stay placed=0 forever (confirmed via bet_history cross-check).
    Prefer the stable event_id+market_url key; fall back to match+label.
    """
    try:
        conn = _get_conn()
        row = None
