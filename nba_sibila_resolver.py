"""Resolve NBA shadow picks in sibila.db via ESPN NBA scoreboard."""
...
def _fetch_espn_nba(date_str: str) -> list:
    if date_str in _ESPN_NBA_CACHE:
        return _ESPN_NBA_CACHE[date_str]
    ...
    try:
        ...
        log.debug('ESPN NBA fetch failed %s: %s', date_str, e)
        _ESPN_NBA_CACHE[date_str] = []
        return []
    ...
    _ESPN_NBA_CACHE[date_str] = results
    return results
...
def _find_nba_result(home: str, away: str, date_str: str):
    try:
    ...
    for delta in [0, 1, -1]:
        ...
        for m in _fetch_espn_nba(check):
...
def resolve_nba_pending(conn: sqlite3.Connection, dry_run: bool = False) -> int:
    """Resolve pending NBA picks in sibila.db. Returns count resolved."""
    rows = conn.execute("""
        SELECT id, side, odds, shadow_stake, real_stake, ts, event_id, market_url
        FROM sibila_picks
        WHERE sport='basketball' AND league='NBA'
          AND result IS NULL
        ORDER BY ts
    """).fetchall()
    ...
    for pick_id, side, odds, stake, real_stake, placed_at, event_id, market_url in rows:
        ...
        team_bet = side.replace('NBA: ', '').strip() if side else ''
        ...
        from oraculo_nba import _resolve_name
        home = _resolve_name(home_cb)
        away = _resolve_name(away_cb)

        result_data = _find_nba_result(home, away, ts)
        if result_data is None:
            log.debug('NBA resolver: no result yet for %s vs %s (%s)', home, away, ts)
            continue
        ...
        log.info('NBA %s | %s vs %s | bet=%s | winner=%s %+.2f',
...
if __name__ == '__main__':
    ...
    n       = resolve_nba_pending(conn, dry_run=dry)
    conn.close()
    print(f'Resolved {n} NBA picks{"(DRY RUN)" if dry else ""}')
